...
KeyPass is a form of ransomware that surfaced in 2018 and targets Windows systems. It encrypts user files with AES and RSA keys and then demands payment in exchange for the decryption key. The malware spreads through malicious downloads or bundled installers and is known for leaving ransom notes named \texttt{!!!DECRYPTION\_KEYPASS\_INFO!!!.txt}. The strings and imports extracted from the sample clearly identify it as ransomware due to the presence of encryption routines, ransom note content, and Windows API usage typical for file manipulation and persistence \cite{keypass-kaspersky}.
...
\subsection{MD5 Hash Confirmation}
\begin{figure}[H] 
  \centering
...
rule Ransomware_KeyPass_Detailed
{
    meta:
...
The \texttt{Botnet\_Mirai\_IoT} rule checks for the ELF32 file format, HTTP request strings such as GET, POST, and Cookie, hardcoded IP addresses linked to Mirai’s command-and-control servers, and XML configuration values. By requiring this combination, the rule is tuned to detect Mirai IoT botnet samples while avoiding false positives on ordinary Linux programs that might only contain one or two of these elements.

\subsubsection{GravityRAT Rule}
\begin{lstlisting}[language=]
rule RAT_GravityRAT_Detection
{
    meta:
...
This rule checks for multiple cryptographic references, exfiltration tags, and host profiling artifacts (Ethernet ID, MAC matching, upload server). By combining encryption and exfiltration behaviors with credential-related strings, the rule specifically identifies GravityRAT samples while reducing false positives on benign .NET programs.  
